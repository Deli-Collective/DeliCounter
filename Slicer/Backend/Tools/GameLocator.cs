        ...
        internal static string GameDirectory
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(steamDir))
                {
                    ...
                    var result = "";
                    if (File.Exists(Path.Combine(steamDir, @"steamapps\appmanifest_450540.acf")))
                    {
                        result = Path.Combine(steamDir, @"steamapps\common\H3VR\");
                    }
                    else
                    {
                        ...
                        foreach (var ii in File.ReadAllLines(libraryFolders).Skip(4)
                            ...
                            .Select(x => x.Split('\t')[3].Trim('"').Replace(@"\\", @"\")).Where(ii =>
                                File.Exists(ii + @"\steamapps\appmanifest_450540.acf")))
                        {
